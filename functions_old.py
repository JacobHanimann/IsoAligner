...
def display_alignment_for_one_gene_from_database(reference_transcript,list_of_gene_objects,index_of_gene,match, mismatch, open_gap_penalty, gap_extension_penalty, exon_length_AA,ID_type='ENSP'):
    ...
    for transcript in list_of_gene_objects[index_of_gene].protein_sequence_isoform_collection:
        if getattr(transcript, ID_type) == reference_transcript:
            continue
        isoform_pattern_check, alignment_reference_fasta, alignment_isoform_fasta = map_FMI_on_COSMIC_Needleman_Wunsch_with_exon_check(reference_protein_sequence, transcript.Protein_isoform, match, mismatch, open_gap_penalty, gap_extension_penalty, exon_length_AA)[4:7]
        percentage_reference, percentage_isoform = calculate_percentage_of_mapped_positions(isoform_pattern_check, reference_protein_sequence, transcript.Protein_isoform)
...
def create_table_for_one_gene_object(chosen_reference,list_of_gene_objects, index_of_gene,chosen_columns, match, mismatch, open_gap_penalty,gap_extension_penalty, exon_length_AA, ID_type="ENSP", one_ID=True):
    ...
    if one_ID: #chosen_reference is a transcript name
        ...
        reference_protein_sequence = list_of_gene_objects[index_of_gene].protein_sequence_isoform_collection[chosen_reference].Protein_isoform #chosen_reference is an index
    ...
    for index,transcript in enumerate(list_of_gene_objects[index_of_gene].protein_sequence_isoform_collection):
        ...
        aminoacids, reference_position_list, isoform_positions_list = map_FMI_on_COSMIC_Needleman_Wunsch_with_exon_check(reference_protein_sequence, transcript.Protein_isoform, match, mismatch, open_gap_penalty, gap_extension_penalty, exon_length_AA)[1:4]
